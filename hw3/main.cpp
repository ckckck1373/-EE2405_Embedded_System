...
void start_record(float origin[], float t[]){
    while(1){
...
void sw_fall_irq(float origin[], float x, float y, float z, float t[]){
    redLED = !redLED;
    origin[0] = x;
    origin[1] = y;
    origin[2] = z;
    tilt_queue.call(&start_record(origin, t));
}
...
int main(){
    ...
    sw2.fall(&sw_fall_irq(origin, t[0], t[1], t[2], t));
    //
